...
RegisterServerEvent('z_tv:add')
AddEventHandler('z_tv:add', function(url, object, coords, scale, offset, volume)
    ...
    TriggerClientEvent('z_tv:update', -1, tosend)
end)

RegisterServerEvent('z_tv:fetch')
AddEventHandler('z_tv:fetch', function()
    local tosend = {}
    ...
    TriggerClientEvent('z_tv:update', source, tosend)
end)

RegisterServerEvent('z_tv:setvolume')
AddEventHandler('z_tv:setvolume', function(id, volume)
    if volume >= 0 and volume <= 10 then
        if data[id] then

            data[id]['Volume'] = volume

            TriggerClientEvent('z_tv:updatevolume', -1, id, volume)

        end
    end
end)

RegisterServerEvent('z_tv:destroy')
AddEventHandler('z_tv:destroy', function(id)
    if data[id] then
        data[id] = nil
        TriggerClientEvent('z_tv:delete', -1, id)
    end
end)