...
RegisterNetEvent('z_tv:update')
AddEventHandler('z_tv:update', function(players)
...
RegisterCommand('tv', function(source, args)
	TriggerEvent("zTV:ToggleNui", {
        ...
        for k, v in pairs(Config['Objects']) do
            ...
            TriggerServerEvent('z_tv:add', youtubeID, v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'], volume)
            ...
            if volume >= 0 then
                ...
                for k, v in pairs(data) do
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                        ...
                        TriggerServerEvent('z_tv:setvolume', k, volume)
                        break
                    end
                end
...
RegisterCommand('destroy', function(src, args)
    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
            TriggerServerEvent('z_tv:destroy', k)
            break
        end
    end
end)

RegisterNetEvent('z_tv:delete')
AddEventHandler('z_tv:delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('z_tv:updatevolume')
AddEventHandler('z_tv:updatevolume', function(id, volume)
    if data[id] then
